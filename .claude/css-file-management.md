...
* Globalの場合、新規作成ファイルは配置したサブディレクトリの `index.css` に繋ぎこむ
* Pagesの場合、基本はページごとのCSSファイルで全てのスタイリングをおこなう
  * 例）about.css → Aboutページのコンテンツ全てをスタイリング
...
* **Global**（`src/styles/global/`）： サイト全域に効くCSS。`global.css` に統合し全ページで読む。
...
Sass 利用時の補助分類（Settings / Tools = `abstracts/`）は、**このリポジトリでは使わない**（プレーンCSS + postcss-import のため）。  
Sass 変数で行うような値の算出は、ネイティブCSSの機能（`color-mix()` 等）を Tokens 内で利用する。
